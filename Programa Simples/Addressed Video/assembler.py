...
def translate(a):
    ...
    if code > space_table["code"]:
        ...
        for i in range(space_table["code"] - code):
            f.append(to_b(0, 8))
    video = len(vtable)
    if video > space_table["ldvideo"]:
        print( "Warning: video data truncated", video)
        vtable = vtable[0:space_table["ldvideo"]] 
    for i in vtable:
        if is_hex(i):
    ...
    if video < space_table["ldvideo"]:
        for i in range(space_table["ldvideo"] - video):
            f.append(to_b(63, 8)) # white fill
    for i in range(space_table["stvideo"]):
        f.append(to_b(63, 8)) # white fill
    for i in range(space_table["unused"]):
        f.append(to_b(63, 8)) # white fill
    print( "Video:", len(vtable), "/", space_table["ldvideo"] )
...
def make_atable(a):
    for i in a:
        if not is_d_command(i) and not is_l_command(i):
            mnemo = i.split(' ')[0]
            if not((mnemo == 'LDRV')|(mnemo == 'STRV')):
                param = i.split(' ')[1]
            else:
                param =  f"{int(i.split(' ')[1]):#08b}"
                param =  param.split('b')[1]
